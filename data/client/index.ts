...
class Client {
  products = {
    ...
    checkAvailability: (data: any) =>
      HttpClient.get<any>(API_ENDPOINTS.ORDERS_CHECKOUT_RENTAL_PRICE, {
  ...
  orders = {
    ...
    downloadInvoice: (input: GenerateInvoiceDownloadUrlInput) =>
      HttpClient.post<any>(`${API_ENDPOINTS.ORDER_INVOICE_DOWNLOAD}`, input),
  };
  users = {
  ...
  notifications = {
    all: (params?: any) =>
      HttpClient.get<any>(API_ENDPOINTS.NOTIFICATION, { ...params }),
    update: (data: any) =>
      // console.log("data", data)
      HttpClient.get<Type>(`notification-read?id=${data.id}`),
  };
}
